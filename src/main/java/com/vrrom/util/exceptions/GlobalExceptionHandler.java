...
public class GlobalExceptionHandler {
    ...
    private ResponseEntity<String> handleLocalDateTypeMismatch(MethodArgumentTypeMismatchException ex) {
        if (ex.getCause() instanceof DateTimeParseException dtpe) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body(String.format("The date value '%s' is not valid. Expected format: yyyy-MM-dd. Error: %s",
                            ex.getValue(), dtpe.getMessage()));
        }
        return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                .body(String.format("Invalid date format for value '%s'. Expected format: yyyy-MM-dd.",
                        ex.getValue()));
    }
